...
const CHANGELOG = [
  {
    ...
    date: '2026.4.20',
    changes: [
      'LV50以降で背景が段階的に変化（ジャングル→火山→雷雲→遺跡→荒野→鉱山→深海→氷河）',
      '伝説キャラ解放（LV150〜）と背景「雷雲」が同期',
      'ヘッダーUIの視認性向上（どの背景でも数値がクッキリ読める）',
      '広告読み込み失敗時の自動リトライ（最大3回・5秒間隔）',
      '復活モーダル改善（広告ロード中でもタップ可能・失敗時はフォールバック報酬）',
      'LV70以降のレベルバランス調整（プテラ・トリケラの出現率UP）',
    ],
  },
  {
    version: 'v5.3.3',
